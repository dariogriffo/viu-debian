...
if [ -z "$VIU_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <viu_version> <build_version>"
    echo "Example: $0 1.6.1 1"
    exit 1
fi

PACKAGE_NAME="viu"
ORIG_TARBALL="${PACKAGE_NAME}_${VIU_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${VIU_VERSION}"

echo "Creating Debian/Ubuntu source packages for viu ${VIU_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/atanunq/viu/archive/refs/tags/v${VIU_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${VIU_VERSION}-${BUILD_VERSION}~${dist}"
...
viu (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${VIU_VERSION}.
